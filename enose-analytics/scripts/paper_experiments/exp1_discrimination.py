...
def _train_cnn_fold(
    ...
    base_tr = np.where(base_tr == 0, 1.0, base_tr)
    X_tr_norm = ((X_train - base_tr) / base_tr).astype(np.float32)

    base_te = X_test[:, :bl, :].mean(axis=1, keepdims=True)
    base_te = np.where(base_te == 0, 1.0, base_te)
    X_te_norm = ((X_test - base_te) / base_te).astype(np.float32)
...
def _eval_cnn_cv(
    ...
    return accuracy_score(y, y_pred_all), y_pred_all
...
def run(ds: PaperDataset, carl_embeddings: np.ndarray | None = None) -> dict:
    ...
    n_classes = len(set(y))
    cnn_acc, cnn_preds = _eval_cnn_cv(X_value_pure, y, n_classes)
    clf_results.append({
