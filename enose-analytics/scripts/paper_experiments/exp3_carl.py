...
class SensorDataset(Dataset):
    ...
    def _augment(self, x: torch.Tensor) -> torch.Tensor:
        """数据增强: 时间平移 + 高斯噪声 + 通道 dropout"""
        T, C = x.shape

        # 时间平移 (±5%)
        shift = int(np.random.uniform(-0.05, 0.05) * T)
        if shift > 0:
            x = torch.cat([x[shift:], x[-1:].expand(shift, -1)], dim=0)
        elif shift < 0:
            x = torch.cat([x[0:1].expand(-shift, -1), x[:shift]], dim=0)
...
def evaluate_downstream(
    ...
    if len(mix_idx) > 10:
        ...
        sss = StratifiedShuffleSplit(n_splits=1, test_size=0.2, random_state=SEED)
        tr, te = next(sss.split(X_mix_emb, y_combo_enc))
    ...
    if len(pure_idx) > 10:
        ...
        sss = StratifiedShuffleSplit(n_splits=1, test_size=0.2, random_state=SEED)
        tr, te = next(sss.split(X_pure_emb, y_tea_enc))
...
def run(ds: PaperDataset, nldi_results: dict | None = None) -> dict:
    ...
    # ── 4. 下游任务评估 ──
    print(f"  下游任务评估...")
    downstream = evaluate_downstream(embeddings, ds)
    ...
    # No SE
    print(f"    消融: No SE...")
    encoder_no_se, _ = train_carl(ds, epochs=CARL_EPOCHS, use_se=False)
    emb_no_se = extract_embeddings(encoder_no_se, ds)
    down_no_se = evaluate_downstream(emb_no_se, ds)
    ablation_results.append({"variant": "CARL (no SE)", **down_no_se})

    # No augmentation
    print(f"    消融: No augment...")
    encoder_no_aug, _ = train_carl(ds, epochs=CARL_EPOCHS, use_augment=False)
    emb_no_aug = extract_embeddings(encoder_no_aug, ds)
    down_no_aug = evaluate_downstream(emb_no_aug, ds)
