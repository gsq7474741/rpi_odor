"""实验2: NLDI 可加性分析 (§3.2) — 核心计算 + 稳健统计检验。

量化10组二元拼配的传感器响应是否符合线性叠加。
稳健统计: 基于 replicate 层面的 bootstrap CI + Wilcoxon 检验。

输出:
  - Fig.2: 响应-比例曲线 (实测 vs 线性预测), 选 3-4 组代表性
  - Fig.3: NLDI 热力图 (10 组合 × 8 通道)
  - Table 3: NLDI 汇总 + 配对 t 检验 p 值 (Bonferroni 校正)
  - Table 3b: NLDI 稳健统计 (bootstrap CI + Wilcoxon)
  - Forest plot: NLDI 均值 + 95% CI
"""
